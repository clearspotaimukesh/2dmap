...
class Orthomosaic:
    ...
    def load_dataset(self):
        ...
        imagePaths = sorted(list(paths.list_images(self.args["images"])))

        with ThreadPoolExecutor() as executor:
            self.images = list(executor.map(self.load_image, imagePaths))
    ...
    def mixer(self):
        if self.debug:
            print(f"[INFO] {len(self.images)} Images have been loaded")
...
class RealTimeSimulator:
    ...
    def simulate(self):
        ...
        if self.orthomosaic.temp_image is None:
            print("[INFO] Initializing temp_image with first image")
...
if __name__ == "__main__":
    orthomosaic = Orthomosaic(debug=True)
    orthomosaic.load_dataset()
    real_time_simulator = RealTimeSimulator(orthomosaic)
    real_time_simulator.run()
